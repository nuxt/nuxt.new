  ...
  <li class="relative w-full h-full flex justify-center items-center p-[1px]">
    <div class="w-full h-full text-xs card rounded-xl z-10 grid grid-cols-1">
      <div class="relative h-[300px]">
        <img :src="theme.image" class="w-full rounded-t-xl object-cover h-[300px]" alt="" />
        ...
        <div v-if="theme.demo"
          ...
          <div class="absolute bottom-12 left-0 right-0  flex justify-center group">
            <UButton label="preview" icon="i-ph-arrow-up-right-bold" :to="theme.demo" size="sm" target="_blank"
              :trailing="true" color="blur" variant="outline"
      ...
      <div
        ...
        <div class="flex flex-col justify-start gap-y-4">
          <UButton :href="`https://github.com/${theme.repo}/tree/${theme.branch}`" size="xs" _target="blank"
        ...
        <div v-if="!theme.shop" class="flex flex-col gap-y-4">
          <div class="grid grid-cols-1 lg:grid-cols-4 justify-between w-full gap-3"
            ...
            <UButton v-if="theme.studio" target="_blank" :to="`https://nuxt.studio/themes/${theme.slug}`" label="Studio" icon="i-simple-icons-nuxtdotjs" variant="outline" color="gray"
        ...
        <UButton v-else target="_blank" :to="theme.shop" variant="outline" color="gray" label="Buy it now" icon="i-ph-arrow-up-right-bold" :trailing="true" size="sm" :ui="{ icon: { size: { sm: 'w-4 h-4' } } }" class="w-full flex justify-center items-center" />
